  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow flex flex-col h-full">
      ...
      <div className="space-y-2 text-xs text-gray-700 flex-grow">
        <div className="flex items-center"><UserIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" /><span>Client: <span className="font-medium">{clientName}</span></span></div>
        {clientContact && (<div className="flex items-center"><PhoneIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" /><span>Contact: <span className="font-medium">{clientContact}</span></span></div>)}
        <div className="flex items-center"><CalendarIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" /><span>Date: <span className="font-medium">{scheduledStartTime.toLocaleDateString()}</span></span></div>
        <div className="flex items-center"><ClockIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" /><span>Time: <span className="font-medium">{scheduledStartTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></span></div>
        {duration !== 'N/A' && (<div className="flex items-center"><ClockIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" /><span>Duration: <span className="font-medium">{duration}</span></span></div>)}
        <div className="flex items-start"><MapPinIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0 mt-0.5" /><span className="break-words">Location: <span className="font-medium">{locationAddress}</span></span></div>
        {price !== undefined && (<div className="flex items-center"><CurrencyDollarIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0" /><span>{priceLabel}: <span className="font-medium text-green-600">₱{price.toFixed(2)}</span></span></div>)}
        {orderDetails?.completion?.notes && (<div className="flex items-start mt-1 pt-1 border-t border-gray-100"><InformationCircleIcon className="h-4 w-4 mr-2 text-gray-400 flex-shrink-0 mt-0.5" /><span className="text-xs italic break-words">Notes: <span className="font-medium not-italic">{orderDetails.completion.notes}</span></span></div>)}
      </div>

      <div className="mt-auto pt-3 border-t border-gray-200 flex flex-wrap gap-2 justify-end">
        {status === 'PENDING' && ( <> <button onClick={handleReject} className="px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded-md transition-colors">Reject</button> <button onClick={handleAccept} className="px-3 py-1.5 text-xs font-medium text-white bg-green-500 hover:bg-green-600 rounded-md transition-colors">Accept</button> </> )}
        {status === 'CONFIRMED' && (
            <>
         ...
         {status === 'IN_PROGRESS' && ( <> <button onClick={handleContactClient} className="px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-md transition-colors">Contact Client</button> <button onClick={handleMarkAsCompleted} className="px-3 py-1.5 text-xs font-medium text-white bg-teal-500 hover:bg-teal-600 rounded-md transition-colors">Mark Completed</button> </> )}
      </div>
    </div>
  );
};
