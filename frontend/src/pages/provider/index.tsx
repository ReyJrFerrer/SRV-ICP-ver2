...
export default function ProviderIndexPage() {
    ...
    const checkProfile = async () => {
      ...
      try {
        ...
        if ('err' in profileResult) {
          ...
          // Profile exists, check if it's a provider profile
          const profile = profileResult.ok;
          if ('ServiceProvider' in profile.role) {
            router.push('/provider/home');
          } else {
            // If not a provider profile, redirect to root
            setError('You need a service provider profile to access this page');
  ...
  return (
    <>
      <Head>
        <title>Provider Portal | Service Provider App</title>
        <meta name="description" content="Access your provider portal" />
      </Head>
